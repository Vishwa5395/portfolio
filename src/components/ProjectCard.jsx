...
import { ExternalLink, Github, Calendar, Tag, Eye, Star, GitBranch } from 'lucide-react';
...
const ProjectCard = ({ project, index = 0 }) => {
  ...
  return (
    <div
      className={`group relative bg-slate-800/50 backdrop-blur-sm rounded-2xl overflow-hidden border border-slate-700/50 hover:border-cyan-500/30 transition-all duration-500 hover:scale-[1.02] hover:-translate-y-2 ${featured ? 'ring-2 ring-cyan-500/20' : ''
        }`}
      style={{
    ...
    >
      ...
      {/* Status Badge */}
      <div className={`absolute top-4 right-4 z-20 px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(status)}`}>
        {status}
      </div>
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        {/* Overlay */}
        <div className={`absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/50 to-transparent transition-all duration-300 ${isHovered ? 'opacity-100' : 'opacity-60'
          }`} />

        {/* Hover Actions */}
        <div className={`absolute inset-0 flex items-center justify-center gap-4 transition-all duration-300 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          }`}>
          {liveUrl && (
            <a
              href={liveUrl}
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {technologies.length > 4 && (
            <span className="px-2 py-1 bg-gradient-to-r from-cyan-500/10 to-purple-500/10 text-cyan-400 text-xs rounded-md border border-cyan-500/30">
      ...
      {/* Hover Glow Effect */}
      <div className={`absolute inset-0 pointer-events-none transition-all duration-500 ${isHovered ? 'shadow-2xl shadow-cyan-500/20' : ''
        }`} />
